...
function SidebarContent({
  ...
  onPick: (s: string, sub?: string, t?: string) => void;
      ...
      {sections.map((section) => {
        ...
        if ('file' in section) {
          ...
          return (
            ...
            >
              <div
                className="flex-shrink-0 rounded-full size-3"
        ...
        return (
          ...
          >
            <CollapsibleTrigger asChild>
              <Button
                variant="ghost"
                className="justify-start gap-2 px-2 py-2 w-full h-auto"
              >
            ...
            <CollapsibleContent className="space-y-1 mt-1 pl-6">
              {section.subsections.map((sub) => {
                ...
                return (
                  <Collapsible
                    key={sub.id}
                    defaultOpen={activeSubsection === sub.id}
                  >
                    <CollapsibleTrigger asChild>
                      <Button
                        variant="ghost"
                        className="justify-start gap-2 px-2 py-1.5 w-full h-auto text-sm"
                      >
...
/** Sidebar de ESCRITORIO: sin posición ni altura propias (el layout ya lo maneja) */
export const Sidebar = () => {
  const { activeSection, activeSubsection, activeTopic, selectNode } = useDocs();
  ...
  return (
    <div className="bg-sidebar border-r h-full">
...
/** Sidebar MÓVIL: drawer fijo que respeta la altura del navbar */
export const MobileSidebar = () => {
  ...
  return (
    <Fragment>
      ...
      <div
        className={cn(
          'fixed left-0 z-[60] w-80 md:hidden bg-sidebar border-r',
          'top-[var(--navbar-h)] h-[calc(100vh-var(--navbar-h))]',
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b">
          <h2 className="font-semibold text-sidebar-foreground">Documentación</h2>
          <Button
            variant="ghost"
            size="icon"
            onClick={closeSidebar}
